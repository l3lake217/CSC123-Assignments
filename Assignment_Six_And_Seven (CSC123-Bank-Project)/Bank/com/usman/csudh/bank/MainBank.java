...
public class MainBank {
	...
	public void run() {
		//new CSVReader().runCSVReader();
		new CSVReader().runHTTPreader();
		Account acc;
		int option = 0;
		...
		try {
			...
			do {
				...
				case 5:
					...
					try {
						Bank.printAccountInformation(ui.readInt(MSG_ACCOUNT_NUMBER));
					}catch(NoSuchAccountException el) {
						this.handleException(ui, el);
					}
					break;
				case 6:
				...
				case 8:
					//ask for current currency, ask for amount, ask for exchange currency, convert.
					Bank.currencyConverter(ui.readToken(MSG_CURRENCY_SELLING), ui.readDouble(MSG_AMOUNT_SELLING), ui.readToken(MSG_CURRENCY_BUYING));
					break;
				case 9:
