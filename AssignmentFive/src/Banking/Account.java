...
public class Account {
	...
	public Account(int accountNumber, String type, Person accountHolder,int odlimit) {
		...
		public boolean deposit(double amount) {
			...
			//Do not deposit funds, Account is closed and balance is greater than zero.
			if((accountOpen==false&&this.balance>0)||(accountOpen==false&&this.balance+amount>0)) {
					this.deposited=false;
			}
			if(amount<0) return false;
			
			//Account open, Deposit funds.
			if(accountOpen) {
				this.balance=this.balance+amount;
			...
			//Deposit funds, if account is closed and balance is less than zero.
			if(accountOpen==false&&this.balance+amount<=0) {
				this.balance=this.balance+amount;
				String tempS = this.transactionID+" : "+"(CREDIT)" +" : "+amount;
				transactions.add(tempS);
